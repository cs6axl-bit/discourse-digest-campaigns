...
module Admin
  class DigestCampaignsController < Admin::AdminController
    ...
    def destroy
      ...
      delete_unsent = truthy_param?(params[:delete_unsent_queue_rows], default: true)
      if delete_unsent
        # Keep all non-queued rows as history; remove ONLY rows that are still queued.
        DB.exec(
          "DELETE FROM #{::DigestCampaigns::QUEUE_TABLE} WHERE campaign_key = :k AND status = 'queued'",
          k: c.campaign_key.to_s
        )
      end
