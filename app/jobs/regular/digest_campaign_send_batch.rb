      ...
      queue_ids.each_with_index do |qid, idx|
        ...
        begin
          message =
            if has_custom_html
              ...
              UserNotifications.digest(
                ...
                campaign_topic_ids: chosen_topic_ids,
                campaign_key: campaign_key.to_s,
                campaign_id: campaign.id,          # ✅ used for email_id generation
                campaign_since: campaign.send_at
              )
            end
          ...
          if SiteSetting.digest_campaigns_update_digest_attempted_at_on_send
            begin
              us = user.user_stat
              us&.update_columns(digest_attempted_at: Time.current)
            rescue => e
              Rails.logger.warn(
                "[digest-campaigns] Failed updating digest_attempted_at for user_id=#{user.id}: #{e.class}: #{e.message}",
              )
            end
          end
