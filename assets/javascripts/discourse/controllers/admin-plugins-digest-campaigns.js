...
export default class AdminPluginsDigestCampaignsController extends Controller {
  ...
  // Per-campaign delete option: delete unsent queue rows when deleting the campaign (default true)
  @tracked deleteUnsentById = {};
  ...
  deleteUnsentChecked(id) {
    return this.deleteUnsentById?.[id] === undefined
      ? true
      : !!this.deleteUnsentById?.[id];
  }
  ...
  @action
  onDeleteUnsentToggle(id, event) {
    const checked = !!event?.target?.checked;
    this.deleteUnsentById = { ...this.deleteUnsentById, [id]: checked };
  }
  ...
  async deleteCampaign(id) {
    ...
    const deleteUnsent =
      this.deleteUnsentById?.[id] === undefined
        ? true
        : !!this.deleteUnsentById?.[id];

    const msg = deleteUnsent
      ? "Delete this campaign and remove all QUEUED rows for it? (Sent/processing/failed history remains.)"
      : "Delete this campaign only? (All queue rows, including queued, will remain.)";
    ...
    try {
      await ajax(`/admin/digest-campaigns/${id}.json`, {
        type: "DELETE",
        data: { delete_unsent_queue_rows: deleteUnsent },
      });
      this.notice = "Deleted.";
      await this.refresh();
    } catch (e) {
