    ...
    def digest(user, opts = {})
      ...
      html =
        if has_custom_html
          ...
          prefs_url = "#{Discourse.base_url}/my/preferences/emails"
          hostname = Discourse.current_hostname.to_s

          unsub_html = <<~HTML
            <div style="font-size:12px; color:#888; line-height:1.4; padding: 10px 0;">
              <div style="margin-bottom:6px;">
                <a href="#{ERB::Util.html_escape(unsub_url)}" style="color:#888; text-decoration:underline;">Unsubscribe</a>
                &nbsp;|&nbsp;
                <a href="#{ERB::Util.html_escape(prefs_url)}" style="color:#888; text-decoration:underline;">Email preferences</a>
              </div>
              <div style="color:#aaa;">Sent from #{ERB::Util.html_escape(hostname)}</div>
            </div>
          HTML
          ...
          <<~HTML
            <div style="max-width: 600px; margin: 0 auto;">
              ...
              #{(ph1.present? || ph2.present?) ? "<div style=\"font-size: 13px; color:#666; line-height: 1.4; padding: 6px 0;\">#{ph1.present? ? "<div>#{ERB::Util.html_escape(ph1)}</div>" : ""}#{ph2.present? ? "<div>#{ERB::Util.html_escape(ph2)}</div>" : ""}</div><hr style=\"border:0; border-top:1px solid #e6e6e6; margin: 10px 0;\" />" : ""}
