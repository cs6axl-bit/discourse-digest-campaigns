...
module ::DigestCampaigns
  ...
  module DigestAppendData
    ...
    # ============================================================
    # Custom HTML campaigns: rewrite ALL <a href> links to /content?u=...
    # (append tracking params BEFORE encoding)
    # Controlled by SiteSetting.digest_campaigns_custom_html_content_redirect_enabled
    # ============================================================
    def self.rewrite_all_links_in_html_to_content_redirector(html, user, email_id, base)
      return html.to_s if html.to_s.empty?
      return html.to_s unless ENABLE_CONTENT_REDIRECTOR_FOR_POST_BODY_LINKS
      return html.to_s if user.nil?
      return html.to_s if email_id.to_s.empty?
      return html.to_s unless Nokogiri
      ...
      doc.css("a[href]").each do |a|
        ...
        # skip if it's already /content on our host
        begin
          u0 = URI.parse(abs0)
          b0 = URI.parse(base)
  ...
  module UserNotificationsExtension
    ...
    def digest(user, opts = {})
      ...
      custom_html_body = opts[:campaign_custom_html_body]
      preheader_line_1 = opts[:campaign_preheader_line_1]
      preheader_line_2 = opts[:campaign_preheader_line_2]
      ...
      if campaign_topic_ids.blank? && !has_custom_html
      ...
      # Optional: up to 3 subject line variants (random per recipient)
      subject_candidates =
      ...
      if subject_candidates.present?
        subject = ::DigestCampaigns::UserNotificationsExtension.smart_trim_preview(subject_candidates.sample, 200)

        if has_custom_html
          ph_lines = [preheader_line_1, preheader_line_2].map { |x| x.to_s.strip }.reject(&:blank?)
          @preheader_text = ph_lines.first || "Campaign"
        elsif first_topic
      ...
      html =
        if has_custom_html
          # -------------------------------------------------
          # HTML-body campaigns wrapper
          # -------------------------------------------------

          # Build an unsubscribe block explicitly.
          # NOTE: "%{unsubscribe_instructions}" only interpolates inside translated email templates;
          # when using html_override it will NOT be replaced.
          unsub_url = "#{Discourse.base_url}/email/unsubscribe/#{@unsubscribe_key}"
          hostname = Discourse.current_hostname.to_s

          unsub_text = ""
unsub_link_text = "Unsubscribe"

if SiteSetting.respond_to?(:digest_campaigns_html_unsubscribe_text)
  unsub_text = SiteSetting.digest_campaigns_html_unsubscribe_text.to_s.strip
end
if SiteSetting.respond_to?(:digest_campaigns_html_unsubscribe_link_text)
...
unsub_html = <<~HTML
  <div style="font-size:12px; color:#000; line-height:1.4; padding: 10px 0; text-align:center;">
    #{ERB::Util.html_escape(unsub_text)} <a href="#{ERB::Util.html_escape(unsub_url)}" style="color:#000; text-decoration:underline;">#{ERB::Util.html_escape(unsub_link_text)}</a>
  </div>
HTML

          # Logo URL selection:
          # 1) explicit override via SiteSetting.digest_campaigns_html_logo_image_url
          # 2) fall back to common Discourse logo settings
          logo_url = nil
          begin
            v = SiteSetting.digest_campaigns_html_logo_image_url.to_s.strip
            logo_url = v if v.present?
          rescue
            logo_url = nil
          end

          if logo_url.blank?
            candidates = %i[email_logo_url logo_small_url logo_url logo_small logo]
            candidates.each do |k|
              next unless SiteSetting.respond_to?(k)
              v = SiteSetting.public_send(k).to_s.strip
              next if v.blank?
              logo_url = v
              break
            end
          ...
          if logo_url.present? && logo_url.start_with?("/")
            logo_url = Discourse.base_url + logo_url
          end

          ph1 = preheader_line_1.to_s.strip
          ph2 = preheader_line_2.to_s.strip

          # Font size for the optional 1-2 preheader lines (admin setting)
          ph_font_size = 13
          begin
            ph_font_size = SiteSetting.digest_campaigns_html_preheader_font_size.to_i
          rescue
            ph_font_size = 13
          end
          ph_font_size = 13 if ph_font_size <= 0
          ph_font_size = 8 if ph_font_size < 8
          ph_font_size = 40 if ph_font_size > 40

          ph_bold = false
          begin
            ph_bold = SiteSetting.digest_campaigns_html_preheader_bold
          rescue
            ph_bold = false
          end
          ...
          ph_underline = false
          begin
            ph_underline = SiteSetting.digest_campaigns_html_preheader_underline
          rescue
            ph_underline = false
          end

          ph_text_style = +""
          ph_text_style << "font-weight:bold;" if ph_bold
          ph_text_style << "text-decoration:underline;" if ph_underline

          home_url = Discourse.base_url

          # Wrapper format:
          # logo (clickable, points to Discourse base_url)
          # divider
          # optional preheader lines
          # divider
          # custom html body
          # divider
          # unsubscribe block
          <<~HTML
            <div style="max-width: 600px; margin: 0 auto;">
              ...
              #{(ph1.present? || ph2.present?) ? "<div style=\"font-size: #{ph_font_size}px; color:#000; line-height: 1.4; padding: 6px 0; text-align:center; #{ph_text_style}\">#{ph1.present? ? "<div>#{ERB::Util.html_escape(ph1)}</div>" : ""}#{ph2.present? ? "<div>#{ERB::Util.html_escape(ph2)}</div>" : ""}</div><hr style=\"border:0; border-top:1px solid #e6e6e6; margin: 10px 0;\" />" : ""}
          ...
          render_to_string(template: "user_notifications/digest", formats: [:html])
        end

      lines = []
      lines << "Campaign: #{@campaign_key}" if @campaign_key.present?
      if has_custom_html
        # Plain-text fallback for custom HTML
        txt = custom_html_body.to_s
        if Nokogiri
          txt = Nokogiri::HTML(txt).text.to_s
        else
          txt = txt.gsub(/<[^>]+>/, " ")
        end
        txt = ::DigestCampaigns::DigestAppendData.normalize_spaces(txt)
        txt = ::DigestCampaigns::UserNotificationsExtension.smart_trim_preview(txt, 1200)
        lines << txt if txt.present?
      else
        lines << ""
      ...
      # Generate a campaign-shaped email_id (20 digits) once per recipient.
      # Used for tracking param appends + /content redirect encoding.
      email_id = ::DigestCampaigns::DigestAppendData.generate_email_id(campaign_id: campaign_id)

      # Optional: rewrite ALL links in custom HTML body campaigns to /content?u=...
      if has_custom_html && SiteSetting.respond_to?(:digest_campaigns_custom_html_content_redirect_enabled) &&
           SiteSetting.digest_campaigns_custom_html_content_redirect_enabled
        html = ::DigestCampaigns::DigestAppendData.rewrite_all_links_in_html_to_content_redirector(
          html,
          user,
          email_id,
          Discourse.base_url
