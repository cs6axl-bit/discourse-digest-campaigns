...
export default class AdminDigestCampaignsController extends Controller {
  @tracked campaigns = [];
  ...
  async enableCampaign(id) {
    ...
    try {
      ...
      this.error = e?.jqXHR?.responseJSON?.errors?.[0] || e?.message || "Enable failed";
    } finally {
      this.busy = false;
    }
  }
  ...
  async disableCampaign(id) {
    ...
    try {
      ...
      this.error = e?.jqXHR?.responseJSON?.errors?.[0] || e?.message || "Disable failed";
    } finally {
      this.busy = false;
    }
  }
  ...
  async deleteCampaign(id) {
    this.clearMessages();
    if (!confirm("Delete this campaign? (Queue rows remain unless you remove them manually.)")) return;
    ...
    try {
      ...
      this.error = e?.jqXHR?.responseJSON?.errors?.[0] || e?.message || "Delete failed";
    } finally {
      this.busy = false;
    }
  }
