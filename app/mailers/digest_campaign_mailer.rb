...
class DigestCampaignMailer < ActionMailer::Base
  ...
  # Action name is intentionally "digest" to better align with digest-style processors.
  def digest(user, topic_ids, campaign_key)
    @user = user
    @campaign_key = campaign_key.to_s
    @topic_ids = Array(topic_ids).map(&:to_i)

    # Digest-style unsubscribe key/link
